...
int main(int argc, char** argv) {
    ...
    //std::thread t1 (&Client::run_client, Client(hostname, username, port));
    //std::thread t1 (&Client::maintain_connection, Client(hostname, username, port));
...
IReply Client::connectToServer(){
    std::string server_info = hostname + ":" + port;
    stub_ = std::unique_ptr<SNSService::Stub>(SNSService::NewStub(
               grpc::CreateChannel(
    ...
    //std::thread t1 (&Client::maintain_connection, this);
...
IReply Client::Follow(const std::string& username2) {
    ...
    } else if (reply.msg() == "unknown follower username" || reply.msg() == "Join Failed -- Invalid Username") {
        ire.comm_status = FAILURE_INVALID_USERNAME;
    } else if (reply.msg() == "you have already joined" || reply.msg() == "Join Failed -- Already Following User") {
        ire.comm_status = FAILURE_ALREADY_EXISTS;
...
IReply Client::findServer() {
    Request request;
    request.set_username(username);
    request.add_arguments("NONE");
    Reply reply;
    ClientContext context;
...
/*void Client::maintain_connection() {
    while (true){
        Request request;
        request.set_username(username);
        request.add_arguments("SERVER_UP");
        Reply reply;
        ClientContext context;
